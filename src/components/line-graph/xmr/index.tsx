...
export class MoneroGraph extends React.Component<Props, State> {
	...
	public formatRawData = (data: any) => {
		return data.filter((_element: any, i: number) => i % 1 === 0).map((arr: number[], i: number) => {
	...
	public render() {
		...
		const diffPercent = oldest < current ? (diff / oldest * 100).toFixed(2) : (diff / current * 100).toFixed(2)
		...
		return !fetchingData && !!data[0] ? (
			<div className="xmr-price">
				...
				<div className="xmr-price-data">
					...
					<div className="flex-spacer" />
					<div className="right">
						<p className="xmr-price-data-high">
							<span className="label">High</span>
							<span className="value">{high.toLocaleString('us-EN', { style: 'currency', currency: 'USD' })}</span>
						</p>
						<p className="xmr-price-data-low">
							<span className="label">Low</span>
							<span className="value">{low.toLocaleString('us-EN', { style: 'currency', currency: 'USD' })}</span>
						</p>
					</div>
				</div>
				<div className="flex-spacer" />
